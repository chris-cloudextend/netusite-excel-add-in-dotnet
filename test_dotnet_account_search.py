#!/usr/bin/env python3
"""
Test script to verify .NET backend account search returns correct account types
...
# The Cloudflare Worker proxy forwards to the .NET backend tunnel
...
def test_account_search(pattern, expected_types, description):
    ...
    try:
        ...
        if count == 0:
            print(f"\n⚠️  No accounts returned - this might be expected if you don't have accounts of these types")
            return True
        ...
        if unexpected_types:
            ...
            print(f"\n   Sample unexpected accounts:")
            for bad_type in sorted(unexpected_types)[:3]:
                sample = accounts_by_type[bad_type][:2]
                for acc in sample:
                    print(f"      [{bad_type}] {acc.get('accountnumber', 'N/A')} - {acc.get('accountname', 'N/A')}")
            return False
        else:
        ...
        print(f"   Make sure the backend server is running and the Cloudflare Worker is deployed")
        return False
...
def main():
    print("="*80)
    print("NETSUITE ACCOUNT SEARCH VERIFICATION (.NET Backend)")
    print("="*80)
    ...
    # Test 4: Bank (exact account type)
    bank_types = ['Bank']
    test4_passed = test_account_search('Bank', bank_types,
                                      'Bank Accounts (Bank exact type)')
    ...
    print(f"Bank search:       {'✅ PASSED' if test4_passed else '❌ FAILED'}")
